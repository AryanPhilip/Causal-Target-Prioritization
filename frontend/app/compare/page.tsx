...
export default async function ComparePage({
  ...
  return (
    ...
    >
      {items.length === 0 ? (
        <div className="rounded-[var(--radius-lg)] border border-dashed border-border bg-bg-muted/40 p-8 text-center">
          <p className="text-fg-muted">
            Pick at least one target from the{" "}
            <Link className="font-medium text-accent underline-offset-4 hover:underline" href={diseaseHref("MONDO:0005101")}>
              disease workspace
            </Link>
            .
          </p>
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {items.map((item) => (
            <section
              className="detail-card rounded-2xl border border-border bg-bg-elevated p-6 shadow-[var(--shadow-elevated)]"
              key={item.targetId}
            >
              ...
              <p className="mt-4 font-display text-3xl font-semibold tabular-nums text-fg">
                {item.overallScore}
              </p>
